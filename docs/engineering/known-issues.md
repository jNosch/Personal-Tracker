...
**New lead, 2026-08-11**: confirmed at least one real, non-hypothetical source — a Claude Code `PostToolUse` hook fires after every `Write`/`Edit` tool call and reformats the file on disk (observed directly: wrote a file, got a "PostToolUse hook modified this file after your edit (likely a formatter)" system reminder, confirmed via re-read the content had reflowed). That's a different mechanism than the file-watcher/reformat-on-save theories above — it only explains noise appearing *after an agent edits a file*, not the bare-`checkout`-with-zero-edits recurrence already documented above, so it's a contributing cause, not the sole one. Worth checking `.claude/settings.json` / `.claude/settings.local.json` for the hook definition next session, and whether its formatter config (quote style, line width) actually matches the repo's own Prettier config — if not, that mismatch alone would explain the churn on every agent-touched file.
